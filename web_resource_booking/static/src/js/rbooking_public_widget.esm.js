...
publicWidget.registry.RBookingFormWidget = publicWidget.Widget.extend({
    ...
    start() {
        const root = this.el;
        const csrf = (window.odoo && window.odoo.csrf_token) ? window.odoo.csrf_token : (document.querySelector('input[name="csrf_token"]') || {}).value;
        let types = [];
        const dataSelect = root.querySelector('#rbooking-types-data');
        if (dataSelect) {
            types = Array.from(dataSelect.querySelectorAll('option')).map((opt) => ({ id: opt.value, display_name: opt.dataset.name }));
        }
